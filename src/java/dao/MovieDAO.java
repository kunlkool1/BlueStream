...
public class MovieDAO {
    ...
    //get all move
    public ArrayList<Movie> getAllMovieByAnimeName(String name) {

        ArrayList<Movie> result = new ArrayList<>();
        ResultSet re = null;
        try {
            con = DBConnect.makeConnection();
            String stm1 = "select * from \"tbMovie\" where \"MovieName\"  like '%" + name + "%'";
            ps = con.prepareStatement(stm1);
            re = ps.executeQuery();
            while (re.next()) {
                String MovieID = re.getString("MovieID");
...
//         con = null;
    ...
    public void DeleteEpi(String MovieID, int epNum) {

        try {
            String query = "delete from \"tbEpisodes\" where \"MovieID\" = '"+MovieID+"' and \"EpNum\" = "+epNum;
            con = new DBConnect().makeConnection();
            ps = con.prepareStatement(query);
...
//    insert Movie
    ...
    public void updateEpisodes(String movieId, int epNum, int duration, String movieLink) {
        String query = "update \"tbEpisodes\"\n"
                ...
                + "\"MovieLink\" = '"+movieLink+"'\n"
                + "where \"MovieID\" = '"+movieId+"' and \"EpNum\" = "+epNum;
...
//        System.out.println(dao.getUserFavorite("c48ca6d8-0e7f-4087-a72f-968c6c82d0fb").toString());
        dao.updateEpisodes("KYB201", 1, 40, "https://vspeed.net/embed/p7adnirfl5dvcgu");
    }
}
